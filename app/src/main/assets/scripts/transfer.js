	...
	function showTextList() {
        ...
        $.getJSON(url, function(data) {
            ...
            for (var i = 0; i < data.length; i++) {
                ...
                var row = $('<div class="file"></div>');
    			row.append('<div class="column text">' + data[i].text +'</div>');
    			row.append('<div class="column copy" title="'+STRINGS.COPY+'"></div>');
                $("#end .files").append(row);
            }
        });
    }

    function copyText(_event){
        var $node =  $(_event.currentTarget);
        var mCopyText = $node.siblings(':first').text();
        doOnCopy(mCopyText)
    }

    function doOnCopy(content){
